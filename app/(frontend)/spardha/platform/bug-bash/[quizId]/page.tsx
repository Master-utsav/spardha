...
export default async function BugBashCompetitionPage({
  params,
}: {
  params: { quizId: string };
}) {
  const quizDetailsAsId = await getQuizIdDetails('bug-bash', params.quizId);
  const enrolledInData = await getUserEnrolledInData('bug-bash', params.quizId);
